...
/* FIX: global JSON middleware (ennen /pay ja muut route:t) */
...
app.get("/print-test", async (req, res) => {
  try {
    ...
    console.log("PRINT ERROR:", err.response?.data || err.message);
    ...
    const line_items = cart.map(item => {
      ...
      if (item.extras && Array.isArray(item.extras)) {
        for (const extra of item.extras) {
          const safeExtra = product.extras?.find(
            e => e.name === extra.name
          );

          if (safeExtra) {
            price += safeExtra.price;
          }
        }
      }
