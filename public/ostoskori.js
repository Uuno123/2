document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderCart() {
    ...
    cart.forEach((item, index) => {
      ...
      const itemTotal =
        (product.price + extrasPrice) * item.quantity;
    ...
    summaryContainer.innerHTML += `
      <div>Kuljetus: ${shippingCost.toFixed(2)} €</div>
    `;
...
function updatePayButton() {
  ...
  const valid =
    ...
    (!document.querySelector('input[name="shipping"]:checked') ||
      document.querySelector('input[name="shipping"]:checked').value !== "delivery"
      ? true
      : isAddressValid());
